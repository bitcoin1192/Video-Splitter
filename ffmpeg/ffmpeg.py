...
def ffmpeg_call(input,name):
    subprocess.run(['ffmpeg','-i',input,'-f','segment','-c','copy','-sn','-reset_timestamps','1',name+'/OUTPUT%d_Orig.mp4','-loglevel','quiet'],shell=False)
...
def ff_stitch(name):
    subprocess.run(['ffmpeg','-f','concat','-safe','0','-i',name+'index','-c','copy','/mnt/volume-sgp1-01/cdn/test.webm'],shell=False)
